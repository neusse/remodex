Create a single horizontal sprite strip for the Codex app digital pet `relay` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Small TV-shaped robot pet, dark grey, thick 2px dark outlines, simple flat cel shading, bright green neon face. No shadows, isolated on clean chroma-key background..
