...
values
  ...
  ('__VERSION__-searches', '__VERSION__', 'Inspect Searches UI', 'Expand the Searches section on an assistant response and verify the source list is readable.', 25, 240, true),
  ('__VERSION__-steering', '__VERSION__', 'Try mobile steering', 'While a turn is running, stage and send a steering message, then report anything confusing in the new controls.', 35, 250, true),
  ('__VERSION__-mobile-pr-draft', '__VERSION__', 'Create mobile PR draft', 'Use Commit + Push + PR or Create PR and verify PR creation/opening from the phone.', 35, 251, true),
  ('__VERSION__-repo-diff-review', '__VERSION__', 'Review repository diff', 'Open the repository diff from the Git header, switch scopes, expand files, and copy a path.', 30, 252, true),
  ('__VERSION__-project-thread', '__VERSION__', 'Create a new project thread', 'Use the project picker flow to choose workspace/runtime context and create a thread.', 30, 253, true),
  ('__VERSION__-user-bubble', '__VERSION__', 'Customize user bubble color', 'Change the user bubble color in Settings and verify the timeline uses it.', 15, 254, true),
  ('__VERSION__-onboarding', '__VERSION__', 'Complete onboarding setup', 'Walk through onboarding, copy setup commands if needed, and continue to QR scanning.', 25, 255, true),
  ('__VERSION__-plan-rendering', '__VERSION__', 'Validate plan rendering', 'Open plan details and verify progress, statuses, and rendered markdown are readable.', 25, 256, true),
  ('__VERSION__-terminal-open', '__VERSION__', 'Open Terminal add-on', 'Open the Android Terminal add-on from the sidebar and verify the profile list or empty state is understandable.', 20, 260, true),
  ('__VERSION__-terminal-profile', '__VERSION__', 'Save a terminal profile', 'Create or edit a terminal SSH profile and report confusing labels, key validation, or setup issues.', 35, 270, true),
  ('__VERSION__-terminal-connect', '__VERSION__', 'Try terminal SSH connect', 'Start an SSH connection from a saved terminal profile and report host trust, passphrase, auth, or network issues.', 40, 280, true),
  ('__VERSION__-composer-basic', '__VERSION__', 'Test composer basics', 'Type, edit, send, and clear a message from the composer.', 20, 290, true),
  ('__VERSION__-composer-mentions', '__VERSION__', 'Test composer mentions', 'Try file, skill, or command-style mentions and report autocomplete issues.', 30, 300, true),
  ('__VERSION__-composer-runtime-controls', '__VERSION__', 'Test runtime controls', 'Change available runtime/model controls and verify the selected state remains visible.', 25, 310, true),
  ('__VERSION__-git-status', '__VERSION__', 'Check git status UI', 'Open git/diff status from a repo thread and verify changed files are shown clearly.', 25, 320, true),
  ('__VERSION__-git-diff-stage', '__VERSION__', 'Test diff staging', 'Inspect a changed file and try staging or unstaging a safe local change.', 40, 330, true),
  ('__VERSION__-new-worktree-chat', '__VERSION__', 'Start a worktree chat', 'Create a new chat using worktree mode and verify branch/cwd context is correct.', 45, 340, true),
  ('__VERSION__-worktree-handoff', '__VERSION__', 'Test worktree handoff', 'Move a safe local task into a managed worktree and report transfer/conflict issues.', 50, 350, true),
  ('__VERSION__-fork-thread', '__VERSION__', 'Fork a thread', 'Fork a safe thread and verify context, title, and project routing remain correct.', 30, 360, true),
  ('__VERSION__-notifications', '__VERSION__', 'Test notification permission', 'Enable or inspect run-completion notifications and report permission issues.', 25, 370, true),
  ('__VERSION__-usage-status', '__VERSION__', 'Check usage status', 'Open the usage/rate limit surface and verify numbers and refresh behavior.', 20, 380, true),
  ('__VERSION__-sidebar-open-thread', '__VERSION__', 'Open a recent thread', 'Use the sidebar to switch threads and verify the selected conversation loads correctly.', 20, 390, true),
  ('__VERSION__-sidebar-search', '__VERSION__', 'Search sidebar threads', 'Search for an existing thread and report missing or confusing results.', 20, 400, true),
  ('__VERSION__-new-chat', '__VERSION__', 'Start a new chat', 'Create a new chat from the sidebar and verify it uses the correct local project context.', 25, 410, true),
  ('__VERSION__-archive-thread', '__VERSION__', 'Check archived chats', 'Open archived chats and verify archived threads display correctly.', 20, 420, true),
  ('__VERSION__-design-mode-open', '__VERSION__', 'Open design mode', 'Open the design workspace and verify the empty state or current project loads.', 25, 430, true),
  ('__VERSION__-design-preview', '__VERSION__', 'Test design preview', 'Generate or open a preview and report rendering, loading, or stale snapshot issues.', 35, 440, true),
  ('__VERSION__-design-export', '__VERSION__', 'Test design export', 'Open export options and verify available formats and labels are understandable.', 30, 450, true)
