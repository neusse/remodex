...
async function downsampleImageWithSips(imagePath, maxPixelDimension, timeoutMs = IMAGE_PREVIEW_TOOL_TIMEOUT_MS) {
  ...
  const sipsCommand = resolveSipsCommand();
  try {
    await execFileAsync(sipsCommand.command, [
      ...sipsCommand.argsPrefix,
      "-Z",
...
function resolveSipsCommand() {
  ...
  const scriptPath = findExecutableOnPath("sips.js");
  if (scriptPath) {
    return {
